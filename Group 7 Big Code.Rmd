...
Rolling Stats Comments:

From December 2015 to January 2018, the four ETFs experienced a general upward trend in their average returns. However, from January 2018 to January 2020, returns declined. A sharp drop occurred in early 2020, coinciding with the onset of the COVID-19 pandemic, which significantly disrupted financial markets. From early 2020 to 2021, returns rebounded, reaching a peak in early 2021. After January 2022, returns declined again, with averages remaining negative during both the COVID-19 period (2020) and from mid-2022 onward.

Between 2015 and 2018, volatility initially increased before declining. After 2018, volatility followed an overall upward trajectory. The most significant surge occurred between February 2020 and January 2022, reflecting heightened uncertainty and market instability caused by the pandemic. Another sharp increase in volatility began in mid-2022, indicating persistent instability in financial markets. Despite some fluctuations, volatility has remained at elevated levels since 2020, suggesting that markets have yet to return to pre-pandemic stability.
The impact of COVID-19 is clearly reflected in both graphs. The sharp decline in average returns and the corresponding spike in volatility in early 2020 illustrate the market's reaction to the pandemic-induced shock.

Given these significant fluctuations, the assumption of covariance stationarity appears unrealistic. The graphs demonstrate substantial time variation in both average returns and volatility, indicating that they do not remain constant over time.

If we want to do more rolling stats, i’ve adapted session 15 code.
...
```{r}
gmvp_unrestricted <- globalMin.portfolio(muhat.vals, cov.mat, shorts = TRUE)
gmvp_unrestricted$weights
gmvp_noshorts <- globalMin.portfolio(muhat.vals, cov.mat, shorts = FALSE)
gmvp_noshorts$weights
```
...
The efficient frontier allowing for short sales has the classic Markowitz bullet shape. The only fund close to the frontier is EWH. The no-shorts efficient frontier lies inside and to the right of the unrestricted frontier and the highest possible target expected return is the mean of EWY.
...
```{r}
tangency_unrestricted <- tangency.portfolio(muhat.vals, cov.mat, risk.free, shorts = TRUE)
tangency_unrestricted$weights
tangency_ns <- tangency.portfolio(muhat.vals, cov.mat, risk.free, shorts = FALSE)
tangency_ns$weights
```

The tangency portfolio allowing shorts is a strange long-short portfolio with a large long leveraged position in EWT (wt 2.1) and a large short position in EWY (wt -1.83). The no-short tangency portfolio is 100\% in EWT . 
