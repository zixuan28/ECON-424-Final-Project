...
projectPrices.daily = project.prices
...
projectReturns.daily = na.omit(Return.calculate(projectPrices.daily, method = "discrete"))
...
```{r Equity-Curve}
smpl = "2018::2023"
project.gret.daily = 1+projectReturns.daily[smpl]
equity.daily = cumprod(project.gret.daily)
colors = c("blue", "red", "green", "orange")

matplot(equity.daily, type="l", col=colors, lwd=2, lty=1, xlab="Days", ylab="Equity Value", main="Simulated 5 Year Equity Curve")
legend("topleft", legend = c(project.symbols), col = colors, lty=1, lwd=2)

project.gret = 1+projectReturns[smpl]
equity = cumprod(project.gret)
colors = c("blue", "red", "green", "orange")

matplot(equity, type="l", col=colors, lwd=2, lty=1, xlab="Months", ylab="Equity Value", main="Simulated 5 Year Equity Curve")
legend("topleft", legend = c(project.symbols), col = colors, lty=1, lwd=2)
```
...
The sharpe slopes estimated do not seem to be precise as all of the standard errors are very large. Only Taiwan's standard error is smaller than the estimated sharpe ratio, but is still large.
...
ann.sharpe
```
The asset's sharpe ratios do not kkvixhave the same ranking as the monthly sharpe ratios
...
```{r, echo=FALSE}
VaR.normal.05 = Value.at.Risk(projectReturns, p=0.05, 
                              method="normal",
                              return.type="cc")
VaR.normal.01 = Value.at.Risk(projectReturns, p=0.01)
VaR.empirical.05 = Value.at.Risk(projectReturns, p=0.05, 
                                 method="empirical",
                                 return.type="cc")
VaR.empirical.01 = Value.at.Risk(projectReturns, p=0.01, 
                                 method="empirical",
                                 return.type="cc")
